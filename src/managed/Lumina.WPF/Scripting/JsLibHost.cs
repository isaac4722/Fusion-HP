...
namespace lumina.wpf.scripting
{
    ...
    /// <summary>
    /// El objeto «jslib» de JScript. AutoDual (LECCIÓN DEL CUARTO RUN DEL TAG:
    /// typeof(jslib)="unknown" — el JScript clásico vincula los miembros de los
    /// ítems con nombre a través del ITypeInfo que el sitio entrega en
    /// GetItemInfo, NO por QI tardío: sin TypeInfo el ítem queda como objeto
    /// COM OPACO y «jslib.log» no es una función). AutoDual hace que el CCW
    /// publique IDispatch CON TypeInfo generado de la clase (patrón clásico
    /// del interop .NET→JScript/VBScript de los 2000; los DispIds se generan en
    /// orden de declaración y el script se re-compila en cada carga, por lo que
    /// la estabilidad entre versiones no es un requisito aquí). Los métodos se
    /// llaman SIEMPRE desde el hilo de UI (el script corre allí).
    /// </summary>
    [ComVisible(true)]
