# Arquitectura Híbrida — LuminaPresentation / LuminaPresentation Suite v3.0.0 «HÍBRIDA»
...
| **Gestión de datos** | **C#** (orquestación) + C++ (motor SQLite vía API) | CRUD, import/export, respaldos; el C# SIEMPRE usa parámetros enlazados (`lumina_db_exec` con `params`) | Lógica de negocio rápida de evolucionar; el binario SQLite vive en el núcleo (sin dependencias extra) |
...
(`lumina_render_preview_png`) y la proyección real ocurre en ventanas nativas del motor.
...
* **Empaquetado**: `LuminaLauncher.exe` (nativo, /MT) detecta el runtime disponible (registro
  `HKLM\SOFTWARE\Microsoft\NET Framework Setup\NDP`) y lanza `LuminaPresentation.exe` (net48) o
  `LuminaPresentation35.exe` (net35). **El usuario nunca instala nada**: en Win7 SP1 corre la variante 3.5
...
* **`native/core/include/lumina/lumina.h`** — API C plana, estable, sin excepciones cruzando la
  frontera, sin objetos CRT compartidos, **todo UTF-8**, errores como códigos (`LUMINA_OK=0…`).
...
* **Eventos** nativo→C#: `LuminaEventFn` (un solo delegado, mantenido vivo con `GCHandle` en C#),
...
* **Biblias (.BIB)** — formato texto con detección automática (ver `docs/bib-format.md`):
  ...
  CP1252, con validación y estadísticas (`lumina_bib_parse`).
