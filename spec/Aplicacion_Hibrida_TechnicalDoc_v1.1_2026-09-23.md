...
>
> **ENMIENDA v4.2.0 (26/09/2026):** **§6.1.3/§6.3.1 (ventana de salida) se COMPLETA con el ciclo integrado estilo PowerPoint**: la ventana de salida nace OCULTA y solo es visible mientras dura la presentación — `output.show`/`output.hide` (ipc.v1) conmutan su VISIBILIDAD (la ventana nunca se destruye: show/hide, no recreación, cero parpadeo). El operador inicia con «Iniciar presentación» (o cualquier envío a pantalla) y termina con «Terminar presentación» o `Esc`. El estado `motor.state` incorpora `outputVisible` para sincronizar GUI/Motor autónomo. La selección de monitor viaja con el NOMBRE de dispositivo (`device`, p. ej. `\\.\DISPLAY2`) además del índice: los órdenes de enumeración de WinForms y del núcleo no están garantizados. Se añade el **Modo Operador** (F8): consola dedicada del operador con «en pantalla» + «siguiente», transporte grande, Negro/Logo/Ocultar y lista de programa (patrón Holy/PTT).
