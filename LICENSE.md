# LICENCIA DE SOLO LECTURA (VIEW-ONLY LICENSE)
## LuminaPresentation Suite — v1.1
### Copyright (c) 2026 Isaac. Todos los derechos reservados.

---

**IMPORTANTE — LEA ANTES DE USAR.** Este software, su código fuente, archivos de
construcción, recursos y documentación (en conjunto, el "Software") son propiedad
exclusiva del autor. Al descargar, ver o utilizar el Software usted acepta los
términos de esta licencia. Si no los acepta, no descargue ni use el Software.

### 1. PERMISO DE SOLO LECTURA DEL CÓDIGO
Se le otorga un permiso limitado, no exclusivo, no transferible y **sin costo** para:
1.1. **Ver y leer** el código fuente con fines de estudio, auditoría o revisión técnica.

### 2. PROHIBICIONES ESTRICTAS
Queda **PROHIBIDO**, salvo autorización previa, expresa y por escrito del autor:
2.1. **Modificar, adaptar o crear obras derivadas** del código fuente, en todo o en parte.
2.2. **Redistribuir, publicar, revender, sublicenciar o compartir** el código fuente o
     versiones modificadas del mismo, por cualquier medio (incluidos repositorios,
     bifurcaciones públicas ("forks"), espejos o CDN).
2.3. **Usar el código fuente** (total o parcial) en otros proyectos, propios o de
     terceros, aunque sean sin fines de lucro.
2.4. Eliminar, ocultar o alterar avisos de copyright, marcas o esta licencia.
2.5. Presentar el Software como obra propia o de terceros ( plagio ).

### 3. USO DE LOS BINARIOS (RELEASES COMPILADOS)
3.1. Los archivos ejecutables y paquetes portables publicados en la sección **Releases**
     de este repositorio pueden **descargarse, instalarse y usarse libremente** para uso
     personal, ministerial o interno de su organización.
3.2. Los binarios se suministran "TAL CUAL", sin garantía alguna de funcionamiento,
     actualización o adecuación a un propósito particular.
3.3. No está permitido aplicar ingeniería inversa, descompilación o desensamblado de
     los binarios, excepto en la medida permitida por la ley aplicable.

### 4. COMPONENTES DE TERCEROS (SE MANTENEN SUS LICENCIAS ORIGINALES)
Este proyecto incorpora, de forma separada y sin ánimo de apropiación, componentes de
terceros con licencias **permisivas** (MIT, BSD, Apache-2.0, Zlib, dominio público,
SIL OFL). Las licencias originales de dichos componentes prevalecen sobre esta
licencia en lo que a dichos componentes respecta. La obligación de esos componentes
es **atribución, no reciprocidad**, y ninguno transfiere derechos sobre el Software.

**La lista completa, normativa y verificada por hash, está en [`DEPENDENCIAS.md`](DEPENDENCIAS.md).**
Resumen de los componentes incluidos en el instalador (versión vigente v4.1.1):
- **C++**: Microsoft WIL 1.0.240803.1 (MIT) · doctest 2.4.11 (MIT, solo pruebas) ·
  SQLite amalgamation 3.45.1 (dominio público) · spdlog 1.12.0 (MIT) ·
  nlohmann/json 3.11.x (MIT, preexistente).
- **C#**: Newtonsoft.Json 13.0.3 (MIT) · NLog 4.7.15 (BSD-3) · PDFsharp 1.50.5147 y
  6.1.1 (MIT) · Microsoft.Extensions.Logging.Abstractions 6.0.0 (MIT) ·
  System.Buffers 4.5.1, System.Memory 4.5.4, System.Numerics.Vectors 4.5.0 y
  System.Runtime.CompilerServices.Unsafe 6.0.0 (MIT, cadena transitiva de
  PDFsharp 6.1.1) · System.Data.SQLite.Core 1.0.118 (dominio público/MIT) ·
  DocumentFormat.OpenXml 2.7.2 (MIT) · Ookii.Dialogs 1.0.0 (BSD-3) ·
  Portable.BouncyCastle 1.8.9 (MIT).
- **Assets**: fuentes Outfit, Cormorant Garamond y Libre Baskerville (SIL OFL 1.1) ·
  Tabler Icons (MIT) · Biblia Reina-Valera 1909 (texto de dominio público).

**Regla de oro permanente:** solo se incorporan componentes permisivos. Queda
**prohibido** incorporar componentes GPL/AGPL/LGPL en código enlazado estáticamente
o derivado, salvo (a) licencia comercial adquirida al autor del componente, o
(b) LGPL con enlace dinámico y cláusula que permita al usuario reemplazar la
biblioteca. La distribución **no incluye** Qt, LibVLC ni ningún otro componente
copyleft.

**Cláusula de actualización:** la lista de componentes de terceros de
`DEPENDENCIAS.md` es **normativa**, se actualiza con cada release y **prevalece
sobre esta sección cualquier divergencia** entre ambos documentos.

### 5. SOLICITUDES DE PERMISO
Para obtener permisos adicionales (modificación, redistribución, integración comercial
o licencias OEM), contacte al propietario del repositorio
`https://github.com/isaac4722/Fusion-HP`.

### 6. TERMINACIÓN
El incumplimiento de cualquiera de los términos anteriores implica la terminación
automática e inmediata de esta licencia, debiendo entonces cesar todo uso y destruir
toda copia del Software en su poder.

### 7. LEY APLICABLE
Esta licencia se interpreta conforme a la legislación aplicable del domicilio del autor,
sin perjuicio de los derechos irrenunciables que asistan al usuario conforme a ley.

---
*"Solo ver" significa exactamente eso: puede leer el código, no tomarlo.*
*Las librerías abiertas que viajan dentro respetan sus licencias originales; la
autoría, el diseño y la propiedad del producto siguen siendo del autor.*
