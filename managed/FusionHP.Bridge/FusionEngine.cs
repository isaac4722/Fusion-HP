...
// ============================================================================
//  FusionEngine.cs : envoltura segura (IDisposable) del FusionHandle nativo.
//
//  Eventos (nativo → C#):
...
//   * FusionEvent dispara EN EL HILO DEL MOTOR (contrato: NO llamar al motor
...
//     de apagado es fusion_destroy (join del hilo de eventos) ANTES de liberar
...
namespace fusion.bridge
{
    ...
    public sealed class FusionEvent
    {
            public readonly int Code;        // FusionEvents.*
        public readonly byte[] Payload;  // crudo (UTF-8 o PNG en PREVIEW); puede ser null

        internal FusionEvent(int code, byte[] payload)
        {
            Code = code;
            Payload = payload;
        }
        ...
        public string Text
        {
            get
            {
                if (Payload == null || Payload.Length == 0 || Code == FusionEvents.Preview)
                    return string.Empty;
                return new UTF8Encoding(false).GetString(Payload);
            }
        }

        public string CodeName { get { return FusionEvents.Name(Code); } }
    }

    /// <summary>Manejador de eventos del motor. Puede ejecutarse en el hilo del motor.</summary>
    public delegate void FusionEventHandler(object sender, FusionEvent e);

    /// <summary>Configuración gestionada del motor (espejo de FusionConfig de fusion.h).</summary>
    ...
    public sealed class FusionEngine : IDisposable
    {
        private IntPtr _handle;                                  // FusionHandle
        private NativeMethods.FusionEventFn _callback;           // RAÍZ del delegado (GC)
        ...
        public event FusionEventHandler EventReceived;

        /// <summary>Evento encolado al SynchronizationContext capturado (hilo de UI), si se pasó uno.</summary>
        public event FusionEventHandler UiEventReceived;

        private FusionEngine(SynchronizationContext uiContext)
        {
        ...
        public static FusionEngine Create(bool headless, FusionEventHandler onEvent)
        {
        ...
        {
            FusionEngine engine = new FusionEngine(uiContext);
        ...
        private void Start(bool headless)
        {
            lock (_createLock)
            {
                if (_handle != IntPtr.Zero || _disposed)
                    throw new InvalidOperationException("FusionEngine ya fue iniciado o liberado.");

                // 1) Delegado enraizado: campo + GCHandle (protección doble).
                _callback = new NativeMethods.FusionEventFn(OnNativeEvent);
                _callbackGuard = GCHandle.Alloc(_callback);

                // 2) Config binaria: structSize = sizeof(FusionConfig) — evolución ABI.
                NativeMethods.FusionConfigNative cfg = new NativeMethods.FusionConfigNative();
                cfg.StructSize = Marshal.SizeOf(typeof(NativeMethods.FusionConfigNative));
                ...
                IntPtr h = NativeMethods.fusion_create(ref cfg);
                if (h == IntPtr.Zero)
                {
                    if (_callbackGuard.IsAllocated) _callbackGuard.Free();
                    _callback = null;
                    throw new FusionException(FusionStatus.ErrArg,
                        "fusion_create devolvió null (¿structSize incompatible o sin memoria?).");
                }
                _handle = h;
            }
        }
        ...
        /// <summary>Versión del núcleo ("FusionCore 3.0.0"). Estático: no necesita handle.</summary>
        public static string Version()
        {
            return BufferHelper.InvokeText(NativeMethods.fusion_version);
        }
        ...
        private void OnNativeEvent(IntPtr user, int code, IntPtr payload, int payloadLen)
        {
            ...
            try
            {
                FusionEventHandler h = EventReceived;
                if (h != null) h(this, ev);
            }
            catch (Exception)
            {
            ...
            if (ctx != null)
            {
                try
                {
                    ctx.Post(delegate
                    {
                        ...
                        FusionEventHandler h = UiEventReceived;
                        if (h != null) h(this, ev);
                    }, null);
                }
                catch (Exception)
        ...
        public int LoadScenario(string json)
        {
            ThrowIfDisposed();
            byte[] b = Utf8(json);
            return NativeMethods.fusion_load_scenario(_handle, b, b.Length);
        }

        public int ShowSlide(int index) { ThrowIfDisposed(); return NativeMethods.fusion_show_slide(_handle, index); }
        public int Next()               { ThrowIfDisposed(); return NativeMethods.fusion_next(_handle); }
        public int Prev()               { ThrowIfDisposed(); return NativeMethods.fusion_prev(_handle); }
        public int Black(bool on)       { ThrowIfDisposed(); return NativeMethods.fusion_black(_handle, on ? 1 : 0); }
        public int Clear()              { ThrowIfDisposed(); return NativeMethods.fusion_clear(_handle); }
        ...
        public int SetTheme(string json)
        {
            ThrowIfDisposed();
            byte[] b = Utf8(json);
            return NativeMethods.fusion_set_theme(_handle, b, b.Length);
        }
        ...
        public string StateJson()
        {
            ...
            return BufferHelper.InvokeText((byte[] outBuf, int cap, out int needed) =>
                NativeMethods.fusion_state_json(h, outBuf, cap, out needed));
        }
        ...
        public int Ping(string msg)
        {
            ThrowIfDisposed();
            byte[] b = Utf8(msg);
            return NativeMethods.fusion_ping(_handle, b, b.Length);
        }
        ...
        {
            ThrowIfDisposed();
            return NativeMethods.fusion_projector_show(_handle, screenIndex, fullscreen ? 1 : 0);
        }

        public int ProjectorHide() { ThrowIfDisposed(); return NativeMethods.fusion_projector_hide(_handle); }
        ...
        public byte[] RenderPreviewPng(int slideIndex)
        {
            ...
            try
            {
                return BufferHelper.InvokeBytes((byte[] outBuf, int cap, out int needed) =>
                    NativeMethods.fusion_render_preview_png(h, slideIndex, outBuf, cap, out needed));
            }
        ...
        {
            ThrowIfDisposed();
            return NativeMethods.fusion_db_open(_handle, Utf8Z(path));
        }

        public int DbClose() { ThrowIfDisposed(); return NativeMethods.fusion_db_close(_handle); }
        ...
        public string DbExec(string sqlJson)
        {
            ...
            int needed;
            int st = NativeMethods.fusion_db_exec(h, sql, buf, buf.Length, out needed);
            if (st == FusionStatus.ErrLimit && IsReadOnlySql(sqlJson))
            {
                int retryCap = needed > 0 ? needed : buf.Length * 4;
                byte[] big = new byte[retryCap];
                st = NativeMethods.fusion_db_exec(h, sql, big, big.Length, out needed);
                if (st == FusionStatus.Ok) return DecodeText(big, needed);
            }
            if (st != FusionStatus.Ok)
                throw new FusionException(st, "fusion_db_exec falló (" +
                    FusionStatus.Name(st) + ").");
        ...
        public static string SongParse(string json)
        {
            byte[] b = Utf8(json);
            return BufferHelper.InvokeText((byte[] outBuf, int cap, out int needed) =>
                NativeMethods.fusion_song_parse(b, b.Length, outBuf, cap, out needed));
        }
        ...
        public static string BibParse(byte[] data, string optionsJson)
        {
            if (data == null || data.Length == 0)
                throw new FusionException(FusionStatus.ErrArg, "Datos .BIB vacíos.");
            byte[] opts = string.IsNullOrEmpty(optionsJson) ? null : Utf8Z(optionsJson);
            return BufferHelper.InvokeText((byte[] outBuf, int cap, out int needed) =>
                NativeMethods.fusion_bib_parse(data, data.Length, opts, outBuf, cap, out needed));
        }
        ...
        {
            return BufferHelper.InvokeText((byte[] outBuf, int cap, out int needed) =>
                NativeMethods.fusion_bible_ref_resolve(Utf8Z(reference), outBuf, cap, out needed));
        }
        ...
        {
            return BufferHelper.InvokeText((byte[] outBuf, int cap, out int needed) =>
                NativeMethods.fusion_chords_transpose(Utf8Z(line), semitones, latin ? 1 : 0,
        ...
        {
            if (_disposed)
                throw new ObjectDisposedException("FusionEngine", "El motor ya fue liberado.");
        }
        ...
        ///   2) fusion_destroy → el núcleo hace JOIN del hilo de eventos (no hay
        ...
        public void Dispose()
        {
            ...
            if (h != IntPtr.Zero) NativeMethods.fusion_destroy(h);   // 2)
